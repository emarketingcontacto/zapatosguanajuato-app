...
<head>
    ...
    {{-- seo --}}
        <title>Fabricante de Calzado - {{$biz->bizName}} | ZapatosGuanajuato.com</title>
        <meta name="description" content="Informacion de {{$bizcat->bizcatName}} - {{$biz->bizName}}, encuentra sus mejores modelos y sus datos de contacto para comenzar a hacer negocios"/>
        <meta name="keywords" content="Contacto directo {{$bizcat->bizcatName}} - {{$biz->bizName}}, Directorio de {$bizcat->bizcatName} de zapatos en Guanajuato"/>
        <link rel="canonical" href="{{URL::asset('fabricantes-calzado-guanajuato/'.$biz->bizSlug )}}">
    {{-- end seo --}}
    ...
    {{-- style --}}
        {{-- local --}}
            ...
            <link rel="stylesheet" href="{{ URL::asset('css/biz.css')}}">
        {{-- end local
        {{-- online --}}
            <link rel="stylesheet" href="{{ URL::asset('css/online/bootstrap-min.css')}}">
                ...
                <link rel="stylesheet" href="{{ URL::asset('css/online/biz-min.css') }}"/>
        {{-- end online --}}
    {{-- end style --}}
    ...
    {{-- scripts --}}
        {{-- local --}}
            ...
            <script src="{{ URL::asset('scripts/factories-detail.js')}}"></script>
            {{-- <script src="{{ URL::asset('js/bootstrap.js')}}"></script> --}}
        {{-- end local --}}
        {{-- online --}}
            {{-- <script src="{{ URL::asset('js/bootstrap-min.js')}}"></script> --}}
            {{-- <script src="{{ URL::asset('js/likes-min.js')}}"></script> --}}
            {{-- <script src="{{ URL::asset('js/factories-detail-min.js')}}"></script> --}}
        {{-- end online --}}
    {{-- end scripts --}}
    ...
    @if (session()->has('ga4_signup_slug') || session()->has('ga4_signup_action'))
            <script>
                // 1 data injection
                var signup_slug = "{{session('ga4_signup_slug')}}";
                var signup_action = "{{session('ga4_signup_action')}}";
                // call tag4 conversion event
                macroConversion(signup_slug, signup_action);
            </script>
            @php
                session()->forget(['ga4_signup_slug', 'ga4_signup_action', 'registration_source']);
            @endphp
    ...
    <x-layout>
        ...
        {{-- banner --}}
        {{-- local --}}
        <div class="biz-hero" style="background-image: url('{{asset('storage/'.$biz->bizImage)}}')">
        {{-- online --}}
                ...
                <div class="biz-row-bizdata">
                    <div class="row" style="margin: 0">
                        {{-- provedor de modelos  --}}
                            ...
                            <div class="biz-info-type-models">
                                {{-- row model category --}}
                                <div class="row">
                    ...
                    <div class="row" style="margin:0">
                        ...
                        <div class="col biz-col-info">
                            @if (auth()->user())
                                <a
                                    data-action="mail"
                                    data-proveedor-name="{{$biz->bizName}}"
                                    data-proveedor-email="{{$biz->bizEmail}}"
                                    onclick="sendIntentData(event)"
                                    href=""
                                    >{{$biz->bizEmail}}
                                </a>
                            @else
                                <button class="button-contacto"
                                    data-action="mail"
                                    data-proveedor-name="{{$biz->bizName}}"
                                    data-proveedor-slug ="{{$biz->bizSlug}}"
                                    onclick="sendIntentDataNotLogged(event)">
                                    Ver Email
                                </button>
                            @endif
                        </div>
                ...
                <div class="biz-row-bizdata" id="divSocials">
                    ...
                    <div class="row">
                        ...
                        <div class="col biz-socialmedia-svg">
                            @if (auth()->user())
                                <a class="biz-socialmedia-svg"
                                    data-action="facebook"
                                    data-proveedor-name="{{$biz->bizName}}"
                                    data-proveedor-facebook ="{{$biz->bizFacebook}}"
                                    onclick="sendIntentData(event)">
                        ...
                        <div class="col biz-socialmedia-svg">
                            @if (auth()->user())
                                <a class="biz-socialmedia-svg"
                                    data-action="web"
                                    data-proveedor-name="{{$biz->bizName}}"
                                    data-proveedor-url ="{{$biz->bizWebsite}}"
                                    onclick="sendIntentData(event)">
                            ...
                            @else
                                <a class="biz-socialmedia-svg"
                                    ...
                                    data-proveedor-name="{{$biz->bizName}}"
                                    data-proveedor-slug="{{$biz->bizSlug}}"
                                    onclick="sendIntendDataNotLogged(event)"
                                    >
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-browser-chrome" viewBox="0 0 16 16">
                                        <path fill-rule="evenodd" d="M16 8a8 8 0 0 1-7.022 7.94l1.902-7.098a3 3 0 0 0 .05-1.492A3 3 0 0 0 10.237 6h5.511A8 8 0 0 1 16 8M0 8a8 8 0 0 0 7.927 8l1.426-5.321a3 3 0 0 1-.723.255 3 3 0 0 1-1.743-.147 3 3 0 0 1-1.043-.7L.633 4.876A8 8 0 0 0 0 8m5.004-.167L1.108 3.936A8.003 8.003 0 0 1 15.418 5H8.066a3 3 0 0 0-1.252.243 2.99 2.99 0 0 0-1.81 2.59M8 10a2 2 0 1 0 0-4 2 2 0 0 0 0 4"/>
                                    </svg>
                        ...
                        <div class="col biz-socialmedia-svg">
                            @if (auth()->user())
                                <a class="biz-socialmedia-svg"
                                    ...
                                    data-proveedor-name="{{$biz->bizName}}"
                                    data-proveedor-whatsapp="{{$biz->bizWhatsApp}}"
                                    onclick="sendIntentData(event)">
        ...
        <div class="biz-modelos-container">
            <hr>

            {{--subcategorySearch--}}
            <x-subcategorysearch :bizID="$biz->bizId" bizSLUG="{{$biz->bizSlug}}" bizcatNAME="{{$bizcat->bizcatName}}"></x-subcategorysearch>
            {{--subcategorySearch--}}
                    ...
                    @foreach ($modelos as $modelo)
                        <div class="card" style="width: 18rem;">
                            ...
                            <div class="card-body-modelo">
                                ...
                                <div class="row">
                                    <div class="col-5">
                                        <p class="card-text-modelo">Temporada:</p>
                                    </div>
                                    <div class="col">
                                ...
                                <div class="row">
                                    <div class="col-5">
                                        <p class="card-text-modelo">Tipo:</p>
                                    </div>
                                    <div class="col">
                                ...
                                <div class="row">
                                    <div class="col-5">
                                        <p class="card-text-modelo">Precio:</p>
                                    </div>
                                    <div class="col">
                                        <p class="card-text-modelo-muted modelo-price">${{$modelo->modelPrice}}.00</p>
                                    </div>
                                </div>
        ...
        {{-- modal --}}
            ...
            <div id="leadCaptureModal" class="dialog-backdrop">
                <div class="dialog-content">
                    <button id="closeModalBtn" class="close-btn" aria-label="Close" onclick="closeModal()">&times;</button>
                    <h3 class="dialog-title">¡Contacto directo!</h3>
                    <p>Ingresa tu correo electrónico para ver los datos de contacto y recibir ofertas de
                        <strong>{{$biz->bizName}}</strong>.
                    </p>
                    <form id="leadCaptureForm" action="{{route('User.create')}}">
                        @csrf
                        @method('post')
                        ...
                        <div class="form-group">
                            <input type="submit" value="Enviar" class="btn btn-modal-submit">
                        </div>
                    </form>
                    <div id="modalMessage" class="message-area" style="display: none;"></div>
                </div>
            </div>
